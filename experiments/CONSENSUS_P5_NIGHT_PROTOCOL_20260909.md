...
**Итог проверен 10 сентября:** вся очередь завершилась 9 сентября в
20:31 MSK. References 597/597, общий анализ 1194 rollout / 6 методов;
P5 smoke 8/8 branches, pilot 288/288 branches и 36/36 strict pools.
[Итоговый разбор](CONSENSUS_AND_P5_RESULTS_20260910.md): consensus не дал
убедительного выигрыша; P5 data gate пройден, но K8 rescue opportunity
равна только 1/36. Новая P5-модель не обучалась и не тестировалась.
Замороженный протокол и все промежуточные срезы ниже сохранены.

**Ресурсная поправка 13:44 MSK:** пользователь разрешил GPU **0-7** только
при отсутствии помех чужим вычислениям. Перезапущен idle-only dispatcher,
PID **2694673**, 128 jobs / 368 rollout сохранены. На 13:45 все карты заняты,
workers ждут. Добавлена проверка отсутствия CUDA-процессов и повторная
проверка idle через 2 секунды. **75 тестов** прошли локально и на сервере.
Метод и очередь этапов прежние; исходный freeze сохранён, SHA-цепочка
расширена отдельной поправкой. [Политика, проверки и ограничения гарантии](GPU07_RESOURCE_POLICY_20260909.md).

**Статус проверен 9 сентября, 13:10-13:13 MSK:** диспетчер PID2181819 и
campaign runner PID2183348 живы. References **367/597 rollout**, 127/357 jobs;
Object/Environment по 150/150, Position 67/297. Последний завершённый job
12:05:11; сейчас ожидание свободных GPU, наших GPU collectors нет. P5 smoke
и pilot ещё не начались. Config/gates ниже остаются frozen. Уточнения только
для следующего этапа: [P5 refinement](P5_TASK_CRITICAL_REFINEMENT_20260909.md),
[новый план приоритетов и оценки оставшейся работы](RESEARCH_PRIORITIES_20260909.md).
...
После поправки 13:44 разрешены **GPU 0-7**. Общая очередь начинает job при
memory.used <256 MiB и utilization <5%, отсутствии CUDA-процессов и двух
idle-проверках с интервалом 2 секунды; чужие процессы не завершаются.
Занятая карта не забирает и не резервирует задания:
...
```bash
nohup .venv-cosmos/bin/python -u scripts/run_consensus_valid_support_night.py --execute --gpus 0,1,2,3,4,5,6,7 >> experiments/campaigns/consensus_p5_night_20260909/sequence.log 2>&1 < /dev/null &
```
