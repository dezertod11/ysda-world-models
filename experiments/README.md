...
**15 сентября, итог исправленного runtime v2:**
[Полная S0: таблица, выводы и видео](P3_RUNTIME_V2_RESULTS_20260915.md).
Завершена13:04МСК:995/995rollout,199/199cases,18smoke; повторный audit пройден.
P3fixed96/199 противH16 93/199, Macro54.10% против53.42%; delta+0.68п.п.,
CI[-3.00;4.36],Holmp=1. Общего преимущества нет, event0/199вмешательств.
Все видео локально. S1/S2 отложены. Ниже исторические v1-результаты.

**15 сентября: полная таблица P3 получена, seed-репликация прервана.**
[Результаты, причины, выводы и локальные видео](P3_BENCHMARK_FINAL_RESULTS_20260915.md).
Main995/995: P3fixed95/199, H16 93/199; Macro53.43% против53.42%,
delta+0.013п.п., CI[-3.67;3.78]. Общего преимущества нет; event0/199
вмешательств. S1:180raw/33matched cases, остановка по no-intervention parity,
не робот-fail; S2 не стартовала.1199видео загружены, новых GPU-run нет.

**15 сентября, 01:04 МСК: досчёт P3 и две seed-репликации запущены.**
[Протокол, время, гипотезы и команда статуса](P3_NIGHT_REPLICATION_PROTOCOL_20260915.md).
Сначала 688 оставшихся main outcomes, затем две полные серии 995+12 каждая.
GPU0-7 idle-only, генерация до08:30, пользовательский срок09:00.
Пороги/методы не изменены; дополнительные seeds проверяют воспроизводимость,
не невиданные задачи. Автоматический CPU-аудит coverage и межseed таблиц.

**15 сентября, 00:57 МСК: SSH восстановлен, P3-серия остановилась.**
[Промежуточная таблица, аудит и выводы](P3_BENCHMARK_PARTIAL_RESULTS_20260915.md).
Скачаны 307/995 исходов; полностью сопоставлены 61/199 конфигураций.
Supervisor и workers отсутствуют, `running` в старом статусе неактуален.
P3fixed: 26/61, H16: 23/61; P3event без вмешательств: 23/61.
Это не итоговая Success/199. Исправлен только экспорт отчёта отдельной копией;
исходные настройки сохранены. Новые GPU-вычисления этой проверкой не запущены.

**14 сентября, вечер: повторный анализ и недоступный свежий статус P3.**
[Отчёт, все восемь конфигураций и сравнение H8/H16](EXPERIMENT_REVIEW_20260914_EVENING.md).
Все 512 локальных recovery outcomes повторно сверены с таблицами. Прирост
P3 положителен суммарно против обоих контролей, но зависит от конфигурации.
Новая серия valid199 не объявлена завершённой: SSH недоступен, локальный
статус от 13:26 устарел. Недостающие исходы не считаются fail.

**14 сентября: финальные recovery-результаты и закрытие общей таблицы P3.**
[Итоги и выводы](RECOVERY_FINAL_RESULTS_20260914.md): main512/512,
timing768/768, oracle128/128; сохранён локальный эффект, перенос слабый.
[Следующий прогон](P3_BENCHMARK_CLOSURE_PROTOCOL_20260914.md):
199конфигураций x5методов, Object/Environment/Position, включая новый event-control.
Статус: `campaigns/p3_benchmark_closure_20260914_v3/sequence_status.json`.
Предыдущие сообщения о незавершённом oracle основаны на устаревшем локальном срезе.
