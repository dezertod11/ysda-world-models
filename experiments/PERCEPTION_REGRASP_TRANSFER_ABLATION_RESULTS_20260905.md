...
**Уточнение после аудита 10 сентября:** [проверка и разбор двух harms](P3C_IMPLEMENTATION_AUDIT_20260910.md).
Числа ниже подтверждены по raw данным. `New/novel/unseen cells` здесь означают
новые относительно P3c intervention cohort: все семь встречались в calibration
локализатора на других init. Это не unseen-cell test всей системы. В двух
harms предмет поднимался до trigger; drop отмечен во время approach full
regrasp. Уменьшение общего прироста не равно снижению абсолютного SR.
