#!/usr/bin/env python3
"""Physical shared-prefix rollouts for timing and renewed eligibility."""
import argparse
import json
...
from timing_eligibility import ARMS, intervene
        ...
        try:
            ...
            for arm in ARMS[offset:] + ARMS[:offset]:
                ...
                outcome.update({k: job[k] for k in ('cell', 'phase', 'task_id', 'init_state_id',
                    'repeat', 'rollout_seed', 'calibration_cell_seen')})
                outcome.update(job_id=job['id'], arm=arm, task_description=description,
                    ...
                    physical_fallback=True, diagnostic_only_simulator_labels=True,
                ...
                print(f'[timing] {job["id"]} {arm} success={success} repair={diag["repair_requested"]}', flush=True)
