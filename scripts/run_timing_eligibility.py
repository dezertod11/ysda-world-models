#!/usr/bin/env python3
"""Autonomous, idle-GPU-only timing ablation with replay-first integrity gate."""
import argparse
...
try:
    from scripts.timing_eligibility import NAME, ARMS, make_jobs, CONTRASTS
    from scripts.p5_repeat_feedback import atomic_json, digest
except ModuleNotFoundError:
    from timing_eligibility import NAME, ARMS, make_jobs, CONTRASTS
...
FILES = ('timing_eligibility.py', 'collect_timing_eligibility.py',
         'run_timing_eligibility.py', 'analyze_timing_eligibility.py')


def prepare():
    import pandas as pd
    source = 'experiments/campaigns/grounded_probe_20260911_v2'
    old = json.loads((ROOT / source / 'config.json').read_text())
    predecessor = ROOT / 'experiments/campaigns/timing_eligibility_20260911/budget.json'
    original_deadline = json.loads(predecessor.read_text())['deadline_epoch']
    jobs = make_jobs(old, pd.read_csv(ROOT / 'experiments/frozen_models/perception_regrasp_20260904/calibration_manifest.csv'))
    for job in jobs:
        if job['phase'] == 'smoke':
            folder = ROOT / source / job['source_phase'] / job['source_id']
            names = ['prefix.npz', 'prefix.json']
            names += [a + ext for a in ARMS[:3] for ext in ('.json', '.npz')]
    ...
    cfg = dict(name=NAME, arms=ARMS, jobs=jobs, contrasts=CONTRASTS,
        ...
        localizer=old['localizer'], trigger=old['trigger'], gpu_candidates=list('12345670'),
        max_workers=7, batch_size=2, hours=6, K=4, generated_horizon=16, executed_suffix_horizon=8,
        prefix_t=72, physical_max_t=280, denoising_steps=5, prediction_mode='parallel',
        primary='delayed_latched_checked_h8', diagnostic_only_arm='delayed_latched_h8',
        automatic_holdout=False, previous_holdout_38_45_unopened=True,
        scientific_scope='timing_and_fresh_eligibility_mechanism_on_12_known_position_cells',
        technical_predecessor='timing_eligibility_20260911',
        technical_fix='restore_fresh_prefix_seed_and_cudnn_mode_after_model_loading',
        hard_deadline_epoch=original_deadline)
    ...
    try:
        ...
        for phase in ('smoke', 'screen'):
                ...
                while not stop.is_set() and time.time() < budget['deadline_epoch']:
                    ...
                    try:
                        ...
                        command = ['bash', '-ec', 'source "$1"; bash "$2" "$3"; source "$1"; cd "$COSMOS_REPO"; exec "$COSMOS_VENV/bin/python" "$4" --batch "$5"',
                            'timing', str(ROOT / 'scripts/cosmos_env_libero_pro.sh'), str(ROOT / 'scripts/prepare_libero_pro_position_variant.sh'),
                            level, str(ROOT / 'scripts/collect_timing_eligibility.py'), str(batch)]
            ...
            if not all(done(j) for j in jobs):
                ...
                return
            subprocess.run([sys.executable, str(ROOT / 'scripts/analyze_timing_eligibility.py'),
...
def main():
    ...
    if not (args.launch or args.execute):
        print('Prepared: 10 replay smoke + 480 prospective screen branches; no automatic holdout')
        return
    if ROOT != REMOTE:
