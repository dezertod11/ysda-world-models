...
[Редакционное распределение результатов](EDITORIAL_SELECTION.md): основной
текст посвящён physical recovery и механизму; самостоятельные selector
ветки остаются в полном отчёте. [Что ещё нужно до ICLR](SUBMISSION_CHECKLIST.md).
Все прямые ограничения P3 и актуальная S0-v2 сохранены в статье/appendix.

**Актуальный результат, 15 сентября: S0-v2 полностью завершена.**
[Таблица, статистика, локальные видео и следующий шаг](../../experiments/P3_RUNTIME_V2_RESULTS_20260915.md).
995/995rollout,199cases; P3fixed96/199, H16 93/199; Macro54.10% и53.42%.
Delta+0.6801п.п.,95%CI[-2.9967;4.3636],9rescue/6harm,Holmp=1.
Общего преимущества нет. Event не вмешался ни разу и точно повторилH16.
Runtime audit пройден; raw artifacts загружены и проверены локально.
Старые v1 counts ниже остаются историческими, не второй seed-репликацией.
Headline scoped16/64→41/64 требует отдельной проверки наv2; broad S0 её
не заменяет. S1/S2 отложены по бюджету; межseed устойчивость не установлена.

**15 сентября, reproducibility update:**
[Причина остановки S1 и snapshot v2](../../experiments/runtime_replay_v2/README.md).
На одинаковых сохранённых actions старый restore расходится из-за
несохранённого MuJoCo warmstart; отдельно исправлены sensor clocks/cache.
Новый CPU replay проверен локально и на сервере. Строгую paired-интерпретацию
исторических v1-результатов нужно перепроверить; labels/counts не изменены.
Подготовлены полные S0/S1/S2-v2, но новых GPU-SR ещё нет. Это инженерное
исправление, а не новый положительный результат метода.

**15 сентября: завершён общий P3 benchmark.**
[Полная таблица, статистика, диагностика и видео](../../experiments/P3_BENCHMARK_FINAL_RESULTS_20260915.md).
995/995rollout: H16 93/199, P3fixed95/199; Macro53.42% и53.43%.
Delta+0.0135п.п.,95%CI[-3.6701;3.7801],9rescue/7harm,Holmp=1.
Position+4.04п.п. компенсируется Object-6п.п.; Environment+2п.п. получены
без физического recovery и совпадают сH8. Event не вмешивался ни разу.
ДополнительнаяS1 неполна:33/199matched cases, technical parity failure;
S2 не началась. Это не три завершённых seed-репликации. Scoped41/64 остаётся
самостоятельным результатом, но не доказательством широкого OOD gain.

Этот файл сохраняет полный исследовательский журнал, включая слабые и
отрицательные результаты. Текущая статья сосредоточена на воспроизводимом
эффекте физического восстановления захвата: [научный сюжет](PAPER_NARRATIVE.md),
[краткие результаты](RESEARCH_SUMMARY.md). Остальные направления собраны
в [отдельном разборе](OTHER_EXPERIMENTS.md); они не удалены из истории.

**Вечерняя проверка 14 сентября:** [результаты по конфигурациям и статус доступа](../../experiments/EXPERIMENT_REVIEW_20260914_EVENING.md).
Все 512 локальных исходов подтверждают прежний aggregate. P3 положителен
в 6/8 клеток относительно H8 и в 5/8 относительно H16; выбор comparator
важен. Свежих данных общей серии P3 нет из-за SSH timeout. Не подменять
её показатели ниже результатами conditional recovery.

**Финальные результаты, 14 сентября:**
[Recovery confirmation и oracle: завершённый разбор](../../experiments/RECOVERY_FINAL_RESULTS_20260914.md).
Репликация RGB recovery:41/64 против16/64 H8, +39.06п.п.; transferx0.3:
1/64 против0/64. Preserve-only не улучшает recovery. Geometry32/32 и
oracle128/128 завершены: GT-XY даёт3/16 против0/16 наtransfer. Сбой затронул
только event-shadow. [Протокол закрытия общей таблицы P3](../../experiments/P3_BENCHMARK_CLOSURE_PROTOCOL_20260914.md).
Ниже исторические срезы; для новых выводов использовать финальный отчёт.
